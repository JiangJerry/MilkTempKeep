...
#define TEMP_ADD_H			282		//补正高温值：54
#define TEMP_ADD_STAY		191		//高温值等待下降值：52
#define TEMP_ADD_L			482		//补正高温低点	
#define TEMP_DEC			200		//低温：40度
#define TEMP_30				453		//30度值
...
//	线性关系计算
	...
	if(TempDir)											//下降？上升?实现红绿滞回
	{
		...
		if((k >= max)&&(TempSta != STAY_45))			//目标温度索引值
		{
	    	TempSta = UP_45;							//45度状态
	    	TempDir = true;								//下降
	    	Stay45Cnt = 0;
		...
		{
	    	TempSta = STAY_45;							//45度状态保持状态
			if(Stay45Cnt > WAIT_TIME)					//时间2分钟内
			{
				max = ObjPositiveTemp[AroudTempIndex] + TEMP_ADD_H;	//补正以后的值
				if(k >= max)
				{
	    	...
	    	max = ObjPositiveTemp[AroudTempIndex] + TEMP_ADD_STAY;
			if(k <= max)								//温度下降2度
			{
	    		TempDir = false;						//转为下降				
			}
    ...
    else
    {
    	...
    	else
    	{
    		...
    		{
    			max = ObjPositiveTemp[AroudTempIndex] - TEMP_30;
	    		if(k >= max)//绿灯度数
	    		{
			    	TempSta = IN_30_45;
			    	TempDirDnFlag = true;				//转绿灯温度
	    			max = max + TEMP_ADD_H;				//因为是补正后的值做恒温输出所以需要加差值
			    	ntc_val = (k - max) * 3;				//比例增加值
			    	ntc_val = 2048 - ntc_val;			//可能出现负数
			    	if(ntc_val > 2045)					//不能搞最大2048，否则出现大周期波形
			    		ntc_val = 2045;
			    	if(ntc_val < 0)
			    		ntc_val = 0;	
	    		}
...
#define LOW_BAT_VALU	2233		//1.8V，3.6V以下报底电压
#define LOW_BAT_VALU_UP	2357		//1.9V，3.8V以上才恢复
//#define LOW_BAT_VALU	2419		//1.65V，3.4V以下报底电压仅测试用
//#define LOW_BAT_VALU_UP	2560		//1.8V，3.6V以上才恢复
